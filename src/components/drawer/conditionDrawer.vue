...
<template>
    <el-drawer :append-to-body="true" title="条件设置" v-model="visible" class="condition_copyer" :show-close="false"
        ...
        <div class="demo-drawer__content">
                        ...
                        <div v-if="item.type == 1">
                            <p :class="conditionConfig.nodeUserList.length > 0 ? 'selected_list' : ''"
                                ...
                                <input type="text" placeholder="请选择具体人员/角色/部门"
                                    v-if="conditionConfig.nodeUserList.length == 0" @click="addConditionRole" />
                            </p>
                        </div>
                        ...
                        <div v-else>
                            <p>
                                <select v-model="item.optType" :style="'width:' + (item.optType == 6 ? 370 : 100) + 'px'"
                                    ...
                                    <option v-for="{ value, label } in optTypes" :value="value" :key="value">
                                        {{ label }}
                                    </option>
                                </select>
                            ...
                            <p v-if="item.optType == 6">
                                ...
                                <span class="ellipsis"
                                    style="display: inline-block; width: 60px; vertical-align: text-bottom">{{ item.showName
                                    }}</span>
            ...
            <employees-role-dialog v-model:visible="conditionRoleVisible" :data="checkedList" @change="sureConditionRole"
                :isDepartment="true" />
...
import { useStore } from '@/stores/flow'
import { optTypes, opt1s } from '@/utils/const'
import { getConditions } from '@/api/flow'
...
const sureCondition = () => {
    ...
    for (var i = 0; i < conditionList.value.length; i++) {
        var { columnId, showName, columnName, showType, columnType, fixedDownBoxValue } =
            conditionList.value[i]
        if (
            $func.toggleClass(conditionConfig.value.conditionList, conditionList.value[i], 'columnId')
        ) {
            continue
        }
        if (columnId == 0) {
    ...
    for (let i = conditionConfig.value.conditionList.length - 1; i >= 0; i--) {
        if (
            !$func.toggleClass(conditionList.value, conditionConfig.value.conditionList[i], 'columnId')
        ) {
